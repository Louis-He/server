...
partialsave = '/Users/hsw/Desktop/T_sample_P.png'
...
def readinfofile():
    ...
    if type == '':
        type = 0
    global colorbar
    colorbar = input("colorbar style(0 for regular, 1 for summer, 2 for winter):");
    if colorbar == '':
        colorbar = 0
    # ============================================# read data
...
for i, j, k, l, m, n, p in zip(x, y, T, station, province, code, codeinfile):
    ...
    if colorbar == '1':
        if k <= 8:
            cs2 = map.scatter(i, j, s=15, marker='o', color='#0000FF')
        if 8 < k and k <= 12:
            cs3 = map.scatter(i, j, s=15, marker='o', color='#006FFF')
        if 12 < k and k <= 16:
            cs4 = map.scatter(i, j, s=15, marker='o', color='#00BFFF')
        if 16 < k and k <= 20:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#00FFFF')
        if 20 <= k and k <= 24:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#4FFFAF')
        ...
        if 28 <= k and k <= 32:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#FFFF00')
        if 32 <= k and k <= 35:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#FFCF00')
        if 35 <= k and k <= 37:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#FF9F00')
        if 37 <= k and k <= 40:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#FF6F00')
        if 40 <= k and k <= 45:
            cs5 = map.scatter(i, j, s=15, marker='o', color='#FF0000')
    ...
    if l == '上海' or l == '北京' or l == '重庆' or l == '石家庄' or l == '郑州' or l == '武汉' or l == '长沙' or l == '南京' \
            ...
            or l == '拉萨' or l == '银川' or l == '南宁' or l == '乌鲁木齐' or l == '呼和浩特'or l == '福州' or l == '广州':
    ...
    #analyzing the status of realtime temperature distribution
...
if colorbar == '1':
    ...
    b = mlines.Line2D([], [], color='#FF0000', marker='o',
                              markersize=5, label='40~45°C',ls='')
    c = mlines.Line2D([], [], color='#FF6F00', marker='o',
                              markersize=5, label='37~40°C',ls='')
    d = mlines.Line2D([], [], color='#FF9F00', marker='o',
                              markersize=5, label='35~37°C',ls='')
    e = mlines.Line2D([], [], color='#FFCF00', marker='o',
                              markersize=5, label='32~35°C',ls='')
    f = mlines.Line2D([], [], color='#FFFF00', marker='o',
                              markersize=5, label='28~32°C',ls='')
    g = mlines.Line2D([], [], color='#7FF77F', marker='o',
                              markersize=5, label='24~28°C',ls='')
    h = mlines.Line2D([], [], color='#4FFFAF', marker='o',
                              markersize=5, label='20~24°C',ls='')
    i = mlines.Line2D([], [], color='#00FFFF', marker='o',
                              markersize=5, label='16~20°C',ls='')
    j = mlines.Line2D([], [], color='#00BFFF', marker='o',
                              markersize=5, label='12~16°C',ls='')
    k = mlines.Line2D([], [], color='#006FFF', marker='o',
                              markersize=5, label='8~12°C',ls='')
    l = mlines.Line2D([], [], color='#0000FF', marker='o',
                              markersize=5, label='<8°C',ls='')
    plt.legend(handles=[a, b, c, d, e, f, g, h, i, j, k, l])
plt.title(title)
...
# set up map projection with
# use low resolution coastlines.
map = Basemap(llcrnrlon=100, llcrnrlat=20, urcrnrlon=130, urcrnrlat=45, \
              rsphere=(6378137.00, 6356752.3142), \
              resolution='l', projection='merc', \
              lat_0=40., lon_0=-20., lat_ts=20.)
# draw coastlines, country boundaries, fill continents.
...
for i, j, k, l, m, n, p, q, r in zip(x, y, T, station, province, code, codeinfile, lons, lats):
    temp = temp+1
    size = size_factor * k / max_T
    if q > 100 and q < 130 and r >20 and r < 45:
        if colorbar == '0':
            if k <= -10.0:
                cs1 = map.scatter(i, j, s=15, marker='o', color='#00008F')
            if -10 < k and k <= -5:
