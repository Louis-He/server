...
time = '2017082715'
url = 'http://dd.weather.gc.ca/observations/xml/ON/hourly/hourly_on_' + time + '_e.xml'
xml = (requests.get(url).text)
#print(xml)
root = et.fromstring(xml)
stations = et.Element("STATION")
info = et.Element("INFO")

for child in root:  # layer two
    for i in child:  # layer three
        #print(i.tag, i.attrib, i.text) #<om:Observation> #
        for obs in i:  # layer three
            if obs.tag=='{http://www.opengis.net/om/1.0}result':
                #print(obs.tag, obs.attrib, obs.text)
                stations.append(obs)
            if obs.tag=='{http://www.opengis.net/om/1.0}metadata':
                #print(obs.tag, obs.attrib, obs.text)
                info.append(obs)
